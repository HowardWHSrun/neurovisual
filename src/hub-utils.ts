// Shared boundaries for externally sourced text, links, search, and hash navigation.
const hubUtils = {
  escapeHtml(value: unknown): string {
    return String(value ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]!));
  },
  sourceHref(value: unknown): string {
    try { const u = new URL(String(value)); return /^https?:$/.test(u.protocol) ? hubUtils.escapeHtml(u.href) : '#'; } catch { return '#'; }
  },
  ...
  parseRoute(hash: string): {route: string; id: string; params: URLSearchParams} {
    ...
    let id=''; try { id=decodeURIComponent(rawId||''); } catch {}
    return {route,id,params};
  },
  sourceDate(value: unknown): string {
