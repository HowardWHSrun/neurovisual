...
(function () {
  ...
  function render(focusMain=false) {
    ...
    if(missingRecord){atlas.hidden=true;content.hidden=false;content.innerHTML=notFound();}
      ...
      if(route==='neuroai')NeuroAI.bind(content,params,(hash,focusId)=>{history.pushState(null,'',hash);render(!focusId);if(focusId)document.getElementById(focusId)?.focus({preventScroll:true});});
