...
(function () {
  ...
  function searchPage(params: URLSearchParams) {
    ...
    return pageHead('SEARCH THE WHOLE HUB', q ? 'Results for “'+q+'”' : 'What would you like to explore?', 'Search ideas, company scaling strategies, topics, resources, technologies, organizations, researchers, programs, career roles, job listings, and paper snapshots.')
  ...
  function render(focusMain=false) {
    ...
    if(missingRecord){atlas.hidden=true;content.hidden=false;content.innerHTML=notFound();}
      ...
      content.innerHTML=route==='ideas'?NeuroIdeas.render(id,params):route==='overview'?overview():route==='methods'?methodsPage():route==='resources'?resourcePage(params):route==='topic'?topicPage(id):route==='learn'?learnPage(id):route==='glossary'?glossaryPage(params):route==='search'?searchPage(params):route==='about'?aboutPage():notFound();
      bindFilters(params);
