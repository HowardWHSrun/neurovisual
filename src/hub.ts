...
(function () {
  ...
  function guideDetails(id: string) {
  ...
  function methodsPage(params:URLSearchParams) {
    ...
    const methodOptions=(chosen:string)=>hubMethods.map(m=>`<option value="${m.id}"${m.id===chosen?' selected':''}>${e(m.name)}</option>`).join('');
    const dimensions:[keyof HubMethod,string][]=[['signal','Measured signal'],['access','Typical access'],['strength','Useful strength'],['limit','Main limitation']];
  ...
  function setWorkspace(view:string,detail=false){
    document.body.dataset.view=view;document.body.dataset.detail=String(detail);
    ...
    document.querySelectorAll<HTMLDetailsElement>('.nav-group').forEach(group=>{
      ...
      if(active)section=group.querySelector('summary strong')?.textContent||section;
    });
    document.getElementById('workspace-section')!.textContent=section;
    if(view==='topic'){
      const topic=topicById(parseRoute().id);
      if(topic)document.getElementById('workspace-page')!.textContent=topic.title;
    }
  }
  ...
  function render(focusMain=false) {
    ...
    document.querySelectorAll<HTMLElement>('.site-nav a').forEach(a=>{const active=a.dataset.route===(route==='topic'?'topics':atlasRoute);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
    ...
    if(missingRecord){atlas.hidden=true;content.hidden=false;content.innerHTML=notFound();}
      ...
      bindFilters(params);
      if(route==='explore')NeuroExplore.bind(content,params,(hash,focusId)=>{history.pushState(null,'',hash);render(!focusId);if(focusId)document.getElementById(focusId)?.focus({preventScroll:true});});
      if(route==='ideas')NeuroIdeas.bind(content,id,params);
      if(route==='labs')NeuroLabs.bind(content,id,params,updateHash);
      if(route==='people')NeuroPeople.bind(content,id,params,(hash,focusId)=>{history.pushState(null,'',hash);render();const target=focusId?document.getElementById(focusId):content.querySelector<HTMLElement>('.pe-page h1');if(!focusId){target?.setAttribute('tabindex','-1');window.scrollTo({top:0,behavior:'instant' as ScrollBehavior});}target?.focus({preventScroll:true});});
  ...
  function bindFilters(params: URLSearchParams){
    ...
    if(guideToggle){
      const sections=Array.from(content.querySelectorAll<HTMLDetailsElement>('.guide-reading-detail'));
    ...
    const liveInput=(input:HTMLInputElement,update:()=>void)=>{let composing=false;input.addEventListener('compositionstart',()=>{composing=true;});input.addEventListener('compositionend',()=>{composing=false;update();});input.addEventListener('input',(event:InputEvent)=>{if(!composing&&!event.isComposing)update();});};
    const form = document.getElementById('resource-filters') as HTMLFormElement;
    if(form){const update=()=>{const p=new URLSearchParams();new FormData(form).forEach((v,k)=>{if(String(v).trim())p.set(k,String(v));});updateHash('#resources'+(p.size?'?'+p.toString():''));};form.addEventListener('submit',ev=>{ev.preventDefault();update();});form.addEventListener('change',event=>{if((event.target as HTMLElement).tagName==='SELECT')update();});liveInput(document.getElementById('resource-query') as HTMLInputElement,update);}
    const glossary=document.getElementById('glossary-filter');
    if(glossary){const update=()=>{const p=new URLSearchParams(params);const v=(document.getElementById('glossary-query') as HTMLInputElement).value;if(v)p.set('q',v);else p.delete('q');updateHash('#glossary'+(p.size?'?'+p.toString():''));};glossary.addEventListener('submit',ev=>{ev.preventDefault();update();});liveInput(document.getElementById('glossary-query') as HTMLInputElement,update);}
    const compare=document.getElementById('method-compare') as HTMLFormElement;
    if(compare)compare.addEventListener('change',()=>{const p=new URLSearchParams();new FormData(compare).forEach((v,k)=>p.set(k,String(v)));updateHash('#methods?'+p.toString());});
    const results=document.getElementById('results-search-form');
    if(results)results.addEventListener('submit',event=>{event.preventDefault();const q=(document.getElementById('results-query') as HTMLInputElement).value.trim();location.hash='#search'+(q?'?q='+encodeURIComponent(q):'');});
  }
  document.querySelector('.skip-link')!.addEventListener('click',ev=>{ev.preventDefault();main.focus();});
  let searchReturnFocus:HTMLElement|null=null,restoreSearchFocus=true,searchComposing=false;
  function searchPreview(){
    const q=search.value.trim();const hits=q?searchRecords().filter(r=>matches(r.title+' '+r.description+' '+(r.keywords||''),q)).sort((a,b)=>Number(b.title.toLowerCase().includes(q.toLowerCase()))-Number(a.title.toLowerCase().includes(q.toLowerCase()))):[];
  ...
  function openSearch(){if(searchDialog.open)return;closeMenu();searchReturnFocus=document.activeElement as HTMLElement;restoreSearchFocus=true;const {route,params}=parseRoute();search.value=route==='search'?params.get('q')||'':'';searchPreview();searchDialog.showModal();document.body.classList.add('search-open');searchTrigger.setAttribute('aria-expanded','true');search.focus();}
  function closeSearch(restore=true){restoreSearchFocus=restore;if(searchDialog.open)searchDialog.close();}
  function submitSearch(){const q=search.value.trim();closeSearch(false);const hash='#search'+(q?'?q='+encodeURIComponent(q):'');if(location.hash===hash)render(true);else location.hash=hash;}
  searchTrigger.addEventListener('click',openSearch);
  document.getElementById('search-close')!.addEventListener('click',()=>closeSearch());
  searchDialog.addEventListener('close',()=>{document.body.classList.remove('search-open');searchTrigger.setAttribute('aria-expanded','false');if(restoreSearchFocus)searchReturnFocus?.focus({preventScroll:true});});
  searchDialog.addEventListener('cancel',event=>{if(searchComposing)event.preventDefault();});
  searchDialog.addEventListener('click',event=>{if(event.target===searchDialog)closeSearch();else {const link=(event.target as HTMLElement).closest('a');if(link)closeSearch(!link.getAttribute('href')?.startsWith('#')||event.metaKey||event.ctrlKey||event.shiftKey);}});
  document.getElementById('global-search-form')!.addEventListener('submit',ev=>{ev.preventDefault();if(!searchComposing)submitSearch();});
  search.addEventListener('compositionstart',()=>{searchComposing=true;});search.addEventListener('compositionend',()=>{searchComposing=false;searchPreview();});
  search.addEventListener('input',(event:InputEvent)=>{if(!searchComposing&&!event.isComposing)searchPreview();});
  ...
  document.addEventListener('keydown',ev=>{const target=ev.target as HTMLElement;if(ev.key==='Escape'&&searchDialog.open){if(searchComposing||ev.isComposing)return;ev.preventDefault();closeSearch();return;}if(ev.key==='/'&&!ev.ctrlKey&&!ev.metaKey&&!/INPUT|TEXTAREA|SELECT/.test(target.tagName)&&!target.isContentEditable){ev.preventDefault();openSearch();}if((ev.metaKey||ev.ctrlKey)&&ev.key.toLowerCase()==='k'){ev.preventDefault();openSearch();}if(ev.key==='Escape'&&sidebar.classList.contains('is-open')){closeMenu();menu.focus();}});
