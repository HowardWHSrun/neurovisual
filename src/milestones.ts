// A readable chronology. Images retain their original dates and captions;
// a related platform image is never presented as evidence of a past event.
const MilestoneExperience = (() => {
  const e = hubUtils.escapeHtml;
  const tx = (en:string, zh:string) => I18N.lang === 'zh' ? zh : en;
  let era = 'all';
  const eras = () => [
    {id:'foundations', label:tx('Foundations','奠基阶段'), range:'1924–1999', title:tx('Learning to listen. Beginning to intervene.','学习记录，开始干预。'), test:(year:number)=>year<2000},
    {id:'circuits', label:tx('Circuit tools','神经环路工具'), range:'2000–2019', title:tx('More precise tools. More possibilities.','更精确的工具，更多的可能。'), test:(year:number)=>year>=2000&&year<2020},
    {id:'translation', label:tx('New interfaces','新型接口'), range:'2020–2026', title:tx('From neural signals to human interaction.','从神经信号到人与世界的交互。'), test:(year:number)=>year>=2020}
  ];
  const imageFor:Record<string,string> = {'2017:net':'lab-rice-xie','2023:ti':'lab-stanford-nptl','2025.6:connexus':'company-paradromics'};
  function imagePanel(m:VisualItem, hero=false, caption='') {
  ...
  function render(host:HTMLElement, milestones:any[], allMilestones:any[], technologies:any[], groups:any, selected:string, detailOpen:boolean, activate:(id:string,key:string)=>void) {
    ...
    if(count)count.textContent=visible.length+tx(' of ',' / ')+allCount+' '+tx('Milestones','项里程碑');
    const cards=(entries:any[])=>entries.map(m=>{
    ...
    host.querySelectorAll<HTMLButtonElement>('[data-ms-era]').forEach(button=>button.addEventListener('click',()=>{
      era=button.dataset.msEra!;
      render(host,milestones,allMilestones,technologies,groups,selected,detailOpen,activate);
      host.querySelector<HTMLButtonElement>(`[data-ms-era="${era}"]`)?.focus({preventScroll:true});
    }));
    host.querySelectorAll<HTMLButtonElement>('[data-milestone-tech]').forEach(button=>button.addEventListener('click',()=>activate(button.dataset.milestoneTech!,button.dataset.milestoneKey!)));
