// Original explanatory artwork. Scientific labels follow hub-guides.ts and
// hub-data.ts; shapes, traces, and layouts are illustrative, never measurements.
const FieldVisuals = (() => {
  const e=hubUtils.escapeHtml, u=hubUtils.sourceHref;
  type Ref={title:string;url:string};
  type Diagram={title:string;description:string;art:string};
  const text=(x:number,y:number,value:string,cls='')=>`<text x="${x}" y="${y}" class="${cls}">${e(value)}</text>`;
  const line=(x1:number,y1:number,x2:number,y2:number,cls='fv-line')=>`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" class="${cls}"/>`;
  const dot=(x:number,y:number,r=5,cls='fv-dot')=>`<circle cx="${x}" cy="${y}" r="${r}" class="${cls}"/>`;
  function arrow(x1:number,y1:number,x2:number,y2:number,cls='') {
  ...
  function box(x:number,y:number,w:number,h:number,labels:string[],cls='') {
  ...
  function figure(d:Diagram,sources:Ref[]=[],extra='',className='') {
  ...
  function topicRefs(id:string):Ref[]{return hubGuides[id]?.sources||[];}
  function waveform(x:number,y:number,variant=0) {
  ...
  function camera(x:number,y:number,label:string) {
  ...
  function poseArt() {
    ...
    return svg('Two calibrated camera views observe body landmarks; matched observations support a three-dimensional reconstruction.',
      `${camera(21,43,'A')}${camera(21,154,'B')}<path d="M70 56 L237 80 M70 56 L236 115 M70 168 L237 80 M70 168 L236 115" class="fv-ray"/><path d="M237 80 L236 115 L215 143 L208 185 M236 115 L257 143 L267 185" class="fv-skeleton"/>${body.map(([x,y])=>dot(x,y,6)).join('')}${text(18,23,'2D views')}${text(208,35,'3D landmarks')}${text(34,233,'Calibration + shared timing','fv-muted')}`);
  }
  function methodDiagram(id:string):Diagram {
    ...
    if(method.id==='pose')art=poseArt();
    else if(method.id==='fmri')art=svg('An MRI scanner measures blood-oxygen-related contrast in brain regions, an indirect proxy for activity.',
      ...
      let sensor='';
      if(method.id==='eeg')sensor=`${line(87,78,87,45)}${line(142,78,142,45)}${dot(87,80,8)}${dot(142,80,8)}${text(38,25,'Scalp electrodes')}`;
      if(method.id==='meg')sensor=`<rect x="70" y="28" width="32" height="28" rx="5" class="fv-sensor"/><rect x="127" y="28" width="32" height="28" rx="5" class="fv-sensor"/><path d="M76 66 Q115 37 153 66 M82 72 Q115 52 147 72" class="fv-ray"/>${text(41,21,'Magnetic sensors')}`;
      if(method.id==='ecog')sensor=`<path d="M78 138 H162 V44 H193" class="fv-contact-wire"/>${[82,105,128,151].map(x=>dot(x,143,5)).join('')}${text(28,27,'Cortical-surface contacts')}`;
      if(method.id==='intracortical')sensor=`<path d="M112 43 H126 V192 L119 207 L112 192Z" class="fv-probe"/>${[151,164,177,190].map(y=>dot(119,y,3,'fv-contact')).join('')}${text(43,26,'Contacts within tissue')}`;
      if(method.id==='fnirs')sensor=`<path d="M71 81 C81 199 151 201 169 81" class="fv-light-path"/>${dot(71,83,8,'fv-warm')}${dot(169,83,8)}${text(26,26,'Light source → detector')}`;
      if(method.id==='calcium')sensor=`<rect x="76" y="74" width="87" height="55" class="fv-cutout"/><path d="M100 30 H139 L129 73 H110Z" class="fv-sensor"/><path d="M112 77 L77 195 H163 L129 77Z" class="fv-optical-cone"/>${[[91,175],[123,186],[148,166]].map(([x,y])=>dot(x,y,7)).join('')}${text(171,39,'Optical')}${text(171,58,'access')}`;
  ...
  function method(id:string){const m=hubMethods.find(x=>x.id===id)||hubMethods[0];return figure(methodDiagram(m.id),[m.source],'','fv-method');}
  function methods(leftId='eeg',rightId='intracortical'){return `<div class="fv-method-pair">${method(leftId)}${method(rightId)}</div>`;}

  function signalDiagram(mode='signals'):Diagram {
    if(mode==='interfaces')return {title:'From tissue to samples',description:'The electrode, reference, acquisition electronics, and timing together define the recording.',art:svg('An electrode measures voltage relative to a reference. Acquisition electronics digitize it into timestamped samples.',
      `<path d="M38 44 H53 V136 L46 148 L38 136Z" class="fv-probe"/>${[85,99,113,127].map(y=>dot(46,y,3,'fv-contact')).join('')}${text(17,179,'Electrode')}${arrow(62,96,135,96)}${box(140,66,76,61,['ADC'])}${arrow(221,96,276,96)}${waveform(267,102,0)}${text(259,179,'Samples')}${text(122,214,'Reference + clock','fv-muted')}${line(46,153,46,205)}${line(46,205,115,205)}`)};
    return {title:'A decoder makes an estimate',description:'Processing connects measured signals to estimated variables. Evaluate the estimate on data held out from fitting.',art:svg('Recorded traces become features, then a decoder estimates an output. Held-out data test generalization.',
      `${waveform(23,75,0)}${waveform(23,113,1)}${text(23,157,'Signal')}${arrow(101,97,133,97)}${box(138,67,81,60,['Features'])}${arrow(224,97,258,97)}${box(263,67,77,60,['Estimate'])}<path d="M296 133 V191 H145" class="fv-ray"/>${text(38,219,'Fit on training data · test separately','fv-muted')}`)};
  }
  function loopDiagram(stimulation=false):Diagram {
    return {title:stimulation?'Target, intervene, measure':'Feedback completes the BCI loop',description:stimulation?'A modeled field, neural response, and functional effect are distinct things to evaluate.': 'A BCI translates measured activity into an action. Feedback informs the user or the system’s next adjustment.',art:svg(stimulation?'An intervention acts on a target. A response is measured, and the intended effect must be evaluated.':'Neural activity is measured and decoded into an action; feedback returns to the user or controller.',
      `${box(19,31,133,55,stimulation?['Intervention']:['Neural activity'])}${arrow(158,59,201,59)}${box(208,31,133,55,stimulation?['Target']:['Decoder'])}${arrow(275,92,275,153)}${box(208,160,133,55,stimulation?['Measure response']:['Action'])}${arrow(201,188,158,188)}${box(19,160,133,55,stimulation?['Evaluate effect']:['Feedback'])}${stimulation?'':arrow(86,154,86,93)}${text(131,131,stimulation?'Evidence':'Closed loop','fv-muted')}`)};
  }
  function modelDiagram():Diagram {
    const cells=[[130,53],[190,59],[151,111],[211,124]];
    return {title:'State assumptions, then test predictions',description:'A model links proposed mechanisms to predicted responses. The illustration is conceptual; no simulation was run.',art:svg('Inputs enter a simplified network model. A predicted response must be compared with observations.',
      `${box(14,77,81,56,['Input'])}${arrow(99,104,117,104)}${line(130,53,190,59)}${line(130,53,151,111)}${line(190,59,151,111)}${line(151,111,211,124)}${line(190,59,211,124)}${cells.map(([x,y])=>dot(x,y,12,'fv-neuron')).join('')}${arrow(228,102,254,102)}${waveform(262,104,2)}${text(116,163,'Circuit model')}${text(265,163,'Response')}${text(44,221,'Assumptions → predictions → tests','fv-muted')}`)};
  }
  function imagingDiagram():Diagram {
    return {title:'Measurement and neural activity differ',description:'Hemodynamic contrast and calcium fluorescence are different proxies. Their timing and interpretation follow the measurement process.',art:svg('Blood-oxygen-related contrast and calcium fluorescence are two different proxies for neural activity. Example traces illustrate different measurement processes.',
      `${box(15,23,145,57,['Hemodynamics'])}${box(201,23,144,57,['Fluorescence'])}<ellipse cx="82" cy="131" rx="41" ry="34" class="fv-tissue"/>${dot(68,122,9,'fv-warm')}${dot(98,141,8)}<rect x="225" y="97" width="96" height="69" rx="8" class="fv-image-field"/>${[[243,118],[275,127],[300,145]].map(([x,y])=>dot(x,y,7)).join('')}<circle cx="275" cy="127" r="16" class="fv-roi"/>${waveform(43,203,1)}${waveform(237,203,1)}${text(34,239,'BOLD / fNIRS','fv-muted')}${text(224,239,'Calcium indicator','fv-muted')}`)};
  }
  function provenanceDiagram():Diagram {
    return {title:'Keep the chain from data to result',description:'Dataset identity, metadata, code, parameters, and evaluation choices make a result traceable.',art:svg('A provenance chain connects dataset identifiers, metadata, analysis code and settings, and an evaluated result.',
      `${box(18,29,136,68,['Dataset','Version & IDs'])}${arrow(160,63,200,63)}${box(206,29,136,68,['Metadata','Units & time'])}${arrow(274,105,274,145)}${box(206,152,136,68,['Workflow','Code & settings'])}${arrow(201,187,160,187)}${box(18,152,136,68,['Result','Split & metric'])}`)};
  }
  function evidenceDiagram(regeneration=false):Diagram {
    if(regeneration)return {title:'Three parts of a biohybrid interface',description:'Electronics, living cells, and host tissue must integrate. Survival, signal access, and functional benefit need separate evidence.',art:svg('A biohybrid interface combines an engineered device, living cells, and host tissue; integration and function must each be evaluated.',
    ...
    return {title:'Performance is one part of evidence',description:'Technical performance, functional outcomes, and user experience answer different questions. Feasibility alone does not establish effectiveness.',art:svg('Three separate evidence areas: technical function, meaningful task performance, and user experience. Evidence is connected to a defined intended use.',
      `${box(108,23,145,51,['Intended use'])}${line(180,81,180,110)}${line(68,110,292,110)}${line(68,110,68,134)}${line(180,110,180,134)}${line(292,110,292,134)}${box(18,139,101,65,['Technical','function'])}${box(130,139,101,65,['Task','outcome'])}${box(242,139,101,65,['User','experience'])}${text(61,236,'Distinct questions · linked evidence','fv-muted')}`)};
  }
  function diagram(id:string):Diagram {
    ...
    if(id==='translation'||id==='regeneration')return evidenceDiagram(id==='regeneration');
    return provenanceDiagram();
  }
  function topic(id:string){return figure(diagram(id),topicRefs(id),'','fv-topic');}
  ...
  function lab(profile:LabProfile) {
