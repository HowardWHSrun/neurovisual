// A focused view of the sourced graph, shared with the wider research network.
const PeopleMap = (() => {
  const e = hubUtils.escapeHtml, u = hubUtils.sourceHref, data = neuroConnectionsData;
  const kinds = ['Person', 'Company', 'Institution', 'Lab'];
  const groups = [['all', 'All relationships'], ['affiliation', 'Workplaces & roles'], ['training', 'Training & mentorship'], ['founding', 'Founding & spinouts'], ['work', 'Work & research']];
  ...
  let cy: any = null, resize: ResizeObserver | undefined, events: AbortController | undefined;

  function viewParams(initial: URLSearchParams) {
    const params = new URLSearchParams(initial);
    if (!['focus', 'relation', 'node', 'edge', 'overview'].some(key => params.has(key))) {
      params.set('focus', 'paul-le-floch'); params.set('depth', '2'); params.set('example', '1');
    }
    return params;
  }

  function relationGroup(x: ConnectionEdge) {
    if (x.relationshipType === 'affiliation' && x.affiliationStatus !== 'historical') return 'affiliation';
    if (/phd|student|postdoc|degree|train|mentor|bachelor|master|doctorate/i.test(x.label)) return 'training';
    if (/^(co[- ]?founded|founded|founder of|founding shareholder|helped build founding team|co-established|spun|spin)|participated in the formation/i.test(x.label)) return 'founding';
    return 'work';
  }
  function href(params: URLSearchParams, updates: Record<string, string> = {}) {
    const next = new URLSearchParams({by: 'people', view: 'map'});
    for (const key of ['relation', 'focus', 'depth', 'node', 'edge', 'overview']) if (params.get(key)) next.set(key, params.get(key)!);
  ...
  function model(params = new URLSearchParams()) {
    ...
    if (focus) {
      ...
      for (let step = 0; step < depth; step++) {
        ...
        edges.forEach(x => { if (before.has(x.from) || before.has(x.to)) { ids.add(x.from); ids.add(x.to); } });
      }
  ...
  const normalized = (text: string) => text.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase();
  ...
  function sourceLinks(sources: ConnectionEdge['sources']) {
  ...
  function sentence(x: ConnectionEdge) {
    return `<strong>${e(nodeById.get(x.from)!.name)}</strong> <span>${e(x.label)}</span> <strong>${e(nodeById.get(x.to)!.name)}</strong>`;
  }
  function starters(params: URLSearchParams) {
    return `<div class="pm-starters">${neuroExplorationPeopleData.paths.map((p: any) => {
  ...
  function welcome(params: URLSearchParams) {
  ...
  function nodeDetail(id: string, params: URLSearchParams) {
    const state = model(params), n = state.nodes.find(n => n.id === id);
    if (!n) return `<p class="pm-empty">This entry is outside the current map. Search for it to open its connections.</p>`;
    ...
    const workplaces = baseEdges.filter(x => x.relationshipType === 'affiliation' && x.affiliationStatus !== 'historical' && x.from === id);
    const peopleHere = baseEdges.filter(x => x.relationshipType === 'affiliation' && x.affiliationStatus !== 'historical' && x.to === id);
    const workplaceContext = workplaces.length ? `<section class="pm-workplaces"><h4>Workplaces &amp; roles</h4>${workplaces.map(x => `<div><a data-pm-nav href="${e(href(params,{focus:x.to,node:x.to,edge:'',relation:'affiliation',depth:'1'}))}"><strong>${e(nodeById.get(x.to)!.name)} →</strong></a><p>${e(x.role || x.label)}${x.affiliationStatus === 'emeritus' ? ' · Emeritus' : ''}</p><a href="${u(x.sources[0].url)}" target="_blank" rel="noopener noreferrer">Role source · ${e(x.reviewed || '')} ↗</a></div>`).join('')}</section>` : '';
    const peopleContext = peopleHere.length ? `<div class="pm-institution-context"><p>${peopleHere.length} researchers have checked roles here. Sharing an institution does not establish collaboration.</p><a data-pm-nav href="${e(href(params,{focus:id,node:id,edge:'',relation:'affiliation',depth:'1'}))}">Show people here →</a></div>` : '';
    return `<button class="pm-panel-back" type="button" data-pm-clear>← Map guide</button><div class="pm-panel-head"><span class="eyebrow">${e(n.kind)}</span><h3>${e(n.name)}</h3><p class="pm-subtitle">${e(n.subtitle)}</p>${n.summary && n.summary !== n.subtitle ? `<p>${e(n.summary)}</p>` : ''}</div>${workplaceContext}${peopleContext}<div class="pm-panel-actions"><a class="pm-focus-action" data-pm-nav href="${e(href(params, {focus: id, node: id, edge: '', depth: '1'}))}">Focus on these connections</a>${fullProfile ? `<a href="${fullProfile.startsWith('#') ? e(fullProfile) : u(fullProfile)}"${fullProfile.startsWith('#') ? '' : ' target="_blank" rel="noopener noreferrer"'}>Open full profile ↗</a>` : ''}</div><h4>${related.length} ${related.length === 1 ? 'relationship' : 'relationships'} in this view</h4><div class="pm-neighbors">${related.map(x => {
  ...
  function edgeDetail(id: string, params: URLSearchParams) {
    const x = model(params).edges.find(x => x.id === id);
    if (!x) return '<p class="pm-empty">This relationship is outside the current map. Clear the filter or return to the map guide.</p>';
    return `<button class="pm-panel-back" type="button" data-pm-clear>← Map guide</button><section class="pm-evidence"><span class="eyebrow">DOCUMENTED RELATIONSHIP</span><div class="pm-evidence-direction"><button type="button" data-pm-node="${e(x.from)}">${e(nodeById.get(x.from)!.name)}</button><span>↓ ${e(x.label)}</span><button type="button" data-pm-node="${e(x.to)}">${e(nodeById.get(x.to)!.name)}</button></div><h3>${e(x.role || x.label)}</h3>${x.relationshipType === 'affiliation' ? `<p class="pm-role-status">${x.affiliationStatus === 'emeritus' ? 'Emeritus affiliation' : x.affiliationStatus === 'historical' ? 'Historical affiliation' : 'Current affiliation'}</p>` : ''}<small>${e(x.date)}</small><p>${e(x.detail)}</p>${x.limit ? `<p class="pm-limit">${e(x.limit)}</p>` : ''}<h4>Original sources</h4>${sourceLinks(x.sources)}</section>`;
  }
  function panel(params: URLSearchParams) {
  ...
  function bind(container: HTMLElement, initial: URLSearchParams, navigate: (hash: string, focusId?: string) => void) {
    ...
    const emphasize = () => {
      ...
      if (edge && state.edges.some(x => x.id === edge)) {
        ...
        selected.removeClass('pm-dim').addClass('pm-selected'); selected.connectedNodes().removeClass('pm-dim').addClass('pm-related');
      } else if (node && state.nodes.some(n => n.id === node)) {
        cy.elements().addClass('pm-dim');
        const selected = cy.getElementById(node);
        selected.closedNeighborhood().removeClass('pm-dim').addClass('pm-related'); selected.addClass('pm-selected');
      }
    };
    const select = (kind: 'node' | 'edge' | '', id = '', keyboard = false) => {
      params.delete('node'); params.delete('edge'); if (kind) params.set(kind, id);
      // Selection updates the panel without rebuilding the graph or losing the viewport.
      history.pushState(null, '', href(params)); detail.innerHTML = panel(params); detail.scrollTop = 0; emphasize();
      const jump = container.querySelector<HTMLButtonElement>('#pm-show-details'); if (jump) jump.hidden = !kind;
      if (keyboard) detail.focus();
    };
    container.addEventListener('click', event => {
      const target = (event.target as Element).closest<HTMLElement>('[data-pm-node],[data-pm-edge],[data-pm-nav],[data-pm-clear]');
      ...
      if (target.hasAttribute('data-pm-nav')) {
        ...
        event.preventDefault(); navigate(target.getAttribute('href')!, 'pm-relation'); return;
      }
      if (target.hasAttribute('data-pm-clear')) select('', '', mouse.detail === 0);
      else if (target.dataset.pmEdge) select('edge', target.dataset.pmEdge, mouse.detail === 0);
      else if (target.dataset.pmNode && !results.contains(target)) select('node', target.dataset.pmNode, mouse.detail === 0);
    }, {signal});
    ...
    const openResult = (id: string) => navigate(href(params, {focus: id, node: id, edge: '', depth: '1', relation: ''}), 'pm-panel');
    const drawResults = () => {
      matches = search(input.value).slice(0, 10); active = -1; input.removeAttribute('aria-activedescendant');
    ...
    input.addEventListener('input', () => { if (!composing) drawResults(); }, {signal});
    ...
    input.addEventListener('keydown', event => {
      ...
      if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); hideSearch(); }
      if (['ArrowDown', 'ArrowUp'].includes(event.key) && matches.length && !results.hidden) {
        event.preventDefault(); active = active < 0 ? (event.key === 'ArrowDown' ? 0 : matches.length - 1) : (active + (event.key === 'ArrowDown' ? 1 : -1) + matches.length) % matches.length;
        results.querySelectorAll<HTMLElement>('[role=option]').forEach((option, i) => option.setAttribute('aria-selected', String(i === active)));
        input.setAttribute('aria-activedescendant', 'pm-result-' + active); results.querySelector('#pm-result-' + active)?.scrollIntoView({block: 'nearest'});
      }
      if (event.key === 'Enter' && matches.length && !results.hidden) { event.preventDefault(); openResult(matches[Math.max(0, active)].id); }
    }, {signal});
    results.addEventListener('click', event => { const button = (event.target as Element).closest<HTMLElement>('[data-pm-node]'); if (button) openResult(button.dataset.pmNode!); }, {signal});
    document.addEventListener('click', event => { if (!(event.target as Element).closest('.pm-search')) hideSearch(); }, {signal});
    container.querySelector('#pm-relation')!.addEventListener('change', event => navigate(href(params, {relation: (event.target as HTMLSelectElement).value, node: state.focus, edge: ''}), 'pm-relation'), {signal});
    ...
    const controls = container.querySelectorAll<HTMLButtonElement>('.pm-map-controls button:not(#pm-show-details)');
    ...
    if (typeof cytoscape === 'undefined') { fallback(); return; }
    const fitPadding = () => canvas.clientWidth < 500 ? 28 : 65;
    try {
      cy = cytoscape({
        container: canvas,
        // Deterministic, distinct starting positions keep the force layout finite.
      ...
      cy.on('tap', 'node', (event: any) => select('node', event.target.id()));
      cy.on('tap', 'edge', (event: any) => select('edge', event.target.data('edgeId')));
      cy.on('tap', (event: any) => { if (event.target === cy) select(''); });
      container.querySelector('#pm-fit')!.addEventListener('click', () => cy?.fit(undefined, fitPadding()), {signal});
      const zoom = (factor: number) => cy?.zoom({level: Math.max(.12, Math.min(2.5, cy.zoom() * factor)), renderedPosition: {x: canvas.clientWidth / 2, y: canvas.clientHeight / 2}});
      ...
      if (typeof ResizeObserver !== 'undefined') {
        ...
        resize = new ResizeObserver(() => {
          if (!cy) return;
          cy.resize();
          if (width !== canvas.clientWidth || height !== canvas.clientHeight) { cy.fit(undefined, fitPadding()); width = canvas.clientWidth; height = canvas.clientHeight; }
        });
        resize.observe(canvas);
      }
      emphasize();
    } catch (_) { if (cy) { cy.destroy(); cy = null; } fallback(); }
  }
