...
const NeuroExplore=(()=>{
 ...
 function countries(params:URLSearchParams){
  if(params.get('view')==='companies')return CompanyLocations.render(params);
 ...
 function bind(container:HTMLElement,_params:URLSearchParams,navigate:(hash:string,focusId?:string)=>void){
  ...
  if(_params.get('by')==='people'){if(PeopleWorkplaces.directory(_params))PeopleWorkplaces.bind(container,_params,navigate);else PeopleMap.bind(container,_params,navigate);}
  if(_params.get('by')==='countries'&&_params.get('view')==='companies')CompanyLocations.bind(container,_params,navigate);
  NeuroVisuals.bind(container);
 }
 function records():HubRecord[]{
  return [...CompanyLocations.records(),...problems().map(p=>({id:'explore-problem-'+p.id,title:p.title,description:p.question+' '+p.summary,kind:'Problem to solve',href:link('problems',{problem:p.id}),keywords:(p.labLinks||[]).map((x:any)=>lab(x.labId)?.name+' '+x.reason).join(' ')})),...countryEntries().map(x=>({id:'explore-country-'+x.country,title:x.country+' — labs & companies',description:countryCount(x)+' mapped in this collection.',kind:'Country guide',href:link('countries',{country:x.country}),keywords:x.labs.map(l=>l.institution+' '+l.name+' '+l.leaders.join(' ')).concat(x.companies.map(c=>c.name+' '+c.summary)).join(' ')})),...paths().map(p=>({id:'explore-path-'+p.id,title:p.title,description:p.summary,kind:'People connection',href:p.href,keywords:(p.people||[]).map((id:string)=>node(id)?.name||'').join(' ')}))];
 }
