...
const NeuroIdeas = (() => {
  ...
  function notebook() {
    return head('AN EVOLVING RESEARCH NOTEBOOK', 'Big ideas, made testable.', 'Observations, evidence, and open questions in neuroengineering. Each note separates what is known from what could be tested next.')
      ...
      <section class="lab-home-entry"><div><span class="eyebrow">FOLLOW THE UNIVERSITY RESEARCH</span><h2>What else is scaling?</h2><p>Compare lab efforts in stable recording, selective stimulation, neural decoding, imaging, and functional recovery.</p></div><a class="button-primary" href="#labs?view=progress">Explore lab progress →</a></section>
  ...
  function companyExplorer(params: URLSearchParams) {
    ...
    return `<section class="nv-company-explorer" aria-label="Visual company guide"><div class="nv-company-selector" role="group" aria-label="Choose a company or research platform">${companies.map((c:any,i:number)=>`<button type="button" id="idea-company-${c.id}" data-idea-param="company" data-value="${c.id}" aria-pressed="${selected.id===c.id}">${NeuroMedia.image(c,true)}<span class="nv-selector-name">${e(c.visual.name)}<i aria-hidden="true">${String(i+1).padStart(2,'0')}</i></span><small>${e(c.visual.route)}</small></button>`).join('')}</div>
