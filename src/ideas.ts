declare var neuroIdeasData: any;

// Content lives in data/ideas.json. Dates and scopes belong to each observation.
const NeuroIdeas = (() => {
  const e = hubUtils.escapeHtml;
  const source = (s: any) => `<a href="${hubUtils.sourceHref(s.url)}" target="_blank" rel="noopener noreferrer">${e(s.title)} ↗</a>`;
  const sources = (items: any[]) => `<ul class="idea-sources">${items.map(s => `<li>${source(s)}</li>`).join('')}</ul>`;
  const href = (id: string) => '#ideas/' + encodeURIComponent(id);
  const ideas = neuroIdeasData.ideas;
  const companies = neuroIdeasData.companies;
  const get = (id: string) => ideas.find((x: any) => x.id === id);
  const n = (v: number | null) => v == null ? 'Not established' : v.toLocaleString('en-US');
  const head = (eyebrow: string, title: string, text: string) => `<header class="hub-page-head"><div><div class="eyebrow">${e(eyebrow)}</div><h1>${e(title)}</h1><p>${e(text)}</p></div></header>`;
  const card = (x: any) => `<a class="idea-card" href="${href(x.id)}"><div class="idea-meta"><span>${e(x.theme)}</span><span class="idea-status">${e(x.status)}</span></div><h2>${e(x.title)}</h2><p>${e(x.summary)}</p><span class="idea-card-end">Explore the question <span aria-hidden="true">↗</span></span></a>`;
  function records(): HubRecord[] {
    return [...ideas.map((x: any) => ({id:x.id,title:x.title,description:x.summary,kind:'Idea',href:href(x.id),keywords:[x.question,x.theme,x.observation,x.interpretation,x.organizations.join(' ')].join(' ')})),
      ...companies.map((c: any) => ({id:'strategy-'+c.id,title:c.name+' · '+c.headline,description:c.thought,kind:'Company scaling',href:href('moores-law-bci')+'?section=companies&company='+c.id,keywords:c.mechanism+' '+c.count}))];
  }
  function notebook() {
  ...
  function countExplorer(params: URLSearchParams) {
  ...
  function companyExplorer(params: URLSearchParams) {
    const selected = companies.find((x: any) => x.id === params.get('company')) || companies[0];
    return `<section><div class="idea-panel-heading"><div><span class="eyebrow">HOW COUNTS GROW</span><h2>Different expansion routes</h2></div></div><p>Compare the mechanism, not just the biggest number. These interpretations are research questions, not forecasts or company roadmaps.</p><div class="idea-company-layout"><div class="idea-company-list" role="group" aria-label="Choose a company or research platform">${companies.map((c: any)=>`<button type="button" id="idea-company-${c.id}" data-idea-param="company" data-value="${c.id}" aria-pressed="${selected.id===c.id}"><strong>${e(c.name)}</strong><span>${e(c.mechanism)}</span></button>`).join('')}</div><article class="idea-company-detail"><div class="idea-meta"><span>${e(selected.mechanism)}</span><span>Checked ${e(neuroIdeasData.reviewed)}</span></div><h2>${e(selected.headline)}</h2><div class="idea-company-number"><strong>${e(selected.count)}</strong><span>${e(selected.context)}</span></div><h3>What the evidence says</h3><p>${e(selected.evidence)}</p><h3>Research thought</h3><p>${e(selected.thought)}</p><details><summary>Bottleneck to investigate</summary><p>${e(selected.bottleneck)}</p></details><div class="idea-next-question"><span class="eyebrow">NEXT QUESTION</span><p>${e(selected.question)}</p></div>${sources(selected.sources)}${selected.org?`<a class="idea-profile-link" href="#org/${selected.org}">Open ${e(selected.name)} in the atlas ↗</a>`:''}</article></div></section>`;
  }
  function evidence() {
  ...
  function render(id: string, params = new URLSearchParams()) {
    if (!id) return notebook();
    const x = get(id);
    if (!x) return head('IDEA NOT FOUND','This note is not in the notebook.','Use the notebook index to find an existing question.')+'<a href="#ideas">Back to ideas</a>';
    ...
    return `<a class="back-link" href="#ideas">← All ideas</a>`+head(x.theme,x.title,x.question)
      ...
      + `<details class="idea-history"><summary>How this idea has changed</summary><ul>${x.history.map((h:any)=>`<li><time>${e(h.date)}</time> ${e(h.change)}</li>`).join('')}</ul></details><section class="idea-related"><h2>Keep exploring</h2><div>${x.related.map((id:string)=>`<a href="${href(id)}">${e(get(id)?.title||id)} ↗</a>`).join('')}</div></section>`;
  }
  function bind(container: HTMLElement, id: string, params: URLSearchParams) {
    container.querySelectorAll<HTMLButtonElement>('[data-idea-param]').forEach(b=>b.addEventListener('click',()=>{
      const next = new URLSearchParams(params);next.set(b.dataset.ideaParam!,b.dataset.value!);
      history.replaceState(null,'',href(id)+'?'+next.toString());
      const focusId=b.id; container.innerHTML=render(id,next);bind(container,id,next);document.getElementById(focusId)?.focus({preventScroll:true});
    }));
    const slider=container.querySelector<HTMLInputElement>('#idea-doubling');
    ...
    const budget=container.querySelector<HTMLInputElement>('#idea-budget-channels');
    if(budget){const calculate=()=>{
      const rate=container.querySelector<HTMLInputElement>('#idea-budget-rate')!,bits=container.querySelector<HTMLSelectElement>('#idea-budget-bits')!;
      const out=container.querySelector('#idea-budget-result')!;
      if(!budget.checkValidity()||!rate.checkValidity()||!budget.value||!rate.value){out.textContent='Enter positive whole numbers within the displayed limits.';return;}
    ...
    };container.querySelectorAll('input,select').forEach(el=>el.addEventListener('input',calculate));calculate();}
  }
  function companyForOrg(org: string) { return companies.find((x: any) => x.org === org); }
