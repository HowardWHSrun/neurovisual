...
var NeuroLabs = (() => {
 ...
 function directory(params:URLSearchParams) {
  ...
  if(view==='coverage')return `<div class="lab-page">${hero()}${nav}${coverage()}</div>`;
  if(view==='schools')return `<div class="lab-page">${hero()}${nav}${schoolsPage(params)}</div>`;
 ...
 function bind(container:HTMLElement,id:string,params:URLSearchParams,update:(hash:string)=>void) {
  if(id)return;const form=container.querySelector<HTMLFormElement>('#lab-filters');if(!form)return;
