interface LabSourceReview { checkedAt:string; status:'read'|'abstract-only'|'unavailable'; note:string; }
interface LabAudit { status:'reviewed'|'partial'; summary:string; changes:string[]; latest:{title:string;date:string;url:string}|null; pages:{url:string;title:string;checkedAt:string;status:'read'|'abstract-only'|'unavailable';supports:string;published?:string}[]; }
interface LabSource { id:string; title:string; url:string; kind:string; published?:string; review?:LabSourceReview; }
interface LabProfile {
 ...
 sources:LabSource[]; verified:string; audit?:LabAudit;
}
interface LabSchool { institution:string;title:string;url:string;country:string;region:string;detail:string; review?:LabSourceReview; }
...
var NeuroLabs = (() => {
 ...
 const readLabel=(status:string)=>({'read':'Page content read','abstract-only':'Abstract / publication record read','unavailable':'Page unavailable'}[status]||'Review scope not recorded');
 function sourceReview(s:LabSource){return s.review?`<small class="lab-source-review" data-review="${e(s.review.status)}">${e(readLabel(s.review.status))} · checked ${e(s.review.checkedAt)}</small><p class="lab-source-note">${e(s.review.note)}</p>`:'<small>Source not rechecked in this audit</small>';}
 function auditPanel(l:LabProfile){const a=l.audit;if(!a)return '';return `<section class="lab-audit" aria-label="Website evidence review"><div class="lab-audit-top"><span class="eyebrow">WEBSITE REVIEW · ${e(l.verified)}</span><span class="lab-audit-status">${a.status==='partial'?'Review has unresolved gaps':a.pages.some(p=>p.status==='unavailable')?'Some pages unavailable':'Source review documented'}</span></div><div class="lab-audit-grid"><div><h2>What the source review found</h2><p>${e(a.summary)}</p></div><div class="lab-audit-latest"><span>Newest dated source found</span>${a.latest?`<time>${e(a.latest.date)}</time>${ext(a.latest.url,a.latest.title)}`:'<p>No publication or update date established from the pages read.</p>'}</div></div><details><summary>Review notes &amp; pages checked (${a.pages.length})</summary><ul class="lab-audit-changes">${a.changes.map(c=>`<li>${e(c)}</li>`).join('')}</ul><ol class="lab-audit-pages">${a.pages.map(p=>`<li>${ext(p.url,p.title)}<small data-review="${e(p.status)}">${e(readLabel(p.status))} · ${e(p.checkedAt)}</small><p>${e(p.supports)}</p></li>`).join('')}</ol><p class="lab-audit-scope">This is a dated review of these pages. “Newest found” does not mean every recent paper was located. An abstract review does not establish that the full paper was read.</p></details></section>`;}
 ...
 function selection(params:URLSearchParams) {
  return data.labs.filter(l=>(!params.get('theme')||l.themes.includes(params.get('theme')!))&&(!params.get('school')||l.institution===params.get('school'))&&(!params.get('region')||l.region===params.get('region'))&&hubUtils.matches([l.name,l.institution,l.country,...l.leaders,...l.themes,l.summary,l.approach,...l.methods,...l.signals,...l.projects.map(p=>p.title+' '+p.detail),l.scale.metric,l.scale.detail].join(' '),params.get('q')||'')).sort((a,b)=>a.institution.localeCompare(b.institution)||a.name.localeCompare(b.name));
 }
  ...
  return `${filters(params,'schools')}<div class="lab-results"><p role="status">${entries.length} discovery pages · ${institutions.length} institutions</p>${active?'<a href="#labs?view=schools">Clear filters</a>':'<span>Alphabetical by institution</span>'}</div>
   <div class="lab-school-index">${institutions.map(institution=>{
    ...
    return `<article class="lab-school-row"><header><span class="lab-school-initial" aria-hidden="true">${e(institution.replace(/^University of /,'').charAt(0))}</span><div><span class="lab-school-location">${e(pages[0].country)} · ${e(pages[0].region)}</span><h2>${e(institution)}</h2>${n?`<a class="lab-school-profiles" href="${e(route(new URLSearchParams(),{school:institution}))}">${n} detailed profile${n===1?'':'s'} here →</a>`:'<span class="lab-school-profiles">Institutional discovery source</span>'}</div></header>
     <ul class="lab-school-links">${pages.map(s=>`<li>${ext(s.url,s.title)}<p>${e(s.detail)}</p>${s.review?`<small class="lab-school-review" data-review="${e(s.review.status)}">${e(readLabel(s.review.status))} · ${e(s.review.checkedAt)}</small><p class="lab-source-note">${e(s.review.note)}</p>`:""}</li>`).join('')}</ul></article>`;
 ...
 function directory(params:URLSearchParams) {
  ...
  const results=`<div class="lab-results"><p role="status">${all.length} of ${data.labs.length} profiles${all.length?` · ${(page-1)*24+1}–${Math.min(page*24,all.length)}`:''}</p>${['q','region','school','theme'].some(k=>params.has(k))?`<a href="${e(route(new URLSearchParams(),{view:view==='labs'?'':view}))}">Clear filters</a>`:'<span>Alphabetical by institution · reviewed 15 Sep 2026</span>'}</div>`;
 ...
 function coverage() {
  const audited=data.labs.filter(l=>l.audit),partial=audited.filter(l=>l.audit!.status==='partial'),pages=audited.flatMap(l=>l.audit!.pages),unavailable=pages.filter(p=>p.status==='unavailable').length;
  return `<div class="lab-coverage"><section class="lab-audit-coverage"><span class="eyebrow">THIS WEBSITE REVIEW</span><h2>${audited.length} profiles with a page-by-page evidence trail</h2><p>${data.schools.filter(s=>s.review).length} university discovery pages checked. ${pages.length} lab-page review records; ${unavailable} unavailable-page records. ${partial.length} profiles have unresolved evidence gaps. Review date: ${e(data.reviewed)}.</p><p>Open a profile’s review notes to see the pages read, abstract-only evidence, unavailable pages, and changes made. The wider atlas has separate coverage and older snapshots.</p>${partial.length?`<details><summary>Profiles with unresolved evidence gaps</summary><div class="lab-audit-gaps">${partial.map(l=>`<a href="${href(l.id)}">${e(l.name)} →</a>`).join('')}</div></details>`:""}</section><section class="lab-coverage-method"><span class="eyebrow">THE RESEARCH PROCESS</span><h2>From a roster to a research profile</h2><ol>${data.methodology.map(x=>`<li>${e(x)}</li>`).join('')}</ol></section>
