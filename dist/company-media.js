const NeuroMedia = (() => {
    ...
    const companyUrl = (id) => '#ideas/moores-law-bci?section=companies&company=' + encodeURIComponent(id);
    const themes = { neuralink: '#84d8ec', blackrock: '#d7b57a', precision: '#beadfa', paradromics: '#8edac7', synchron: '#8ebcf9', bisc: '#f5ba96' };
    const tone = (c) => themes[c.id] || '#84d8ec';
    const dots = (cols, rows, x, y, gap, r = 3) => Array.from({ length: cols * rows }, (_, i) => `<circle cx="${x + (i % cols) * gap}" cy="${y + Math.floor(i / cols) * gap}" r="${r}"/>`).join('');
    function diagram(c) {
        let art = '';
        if (c.id === 'neuralink') {
    ...
    function image(c, compact = false) {
    ...
    function watchNotes(c) {
        const v = c.media?.video;
        if (!v)
            return '';
        return `<section class="nv-watch-notes"><div class="nv-section-label"><span>INSIDE THE VIDEO</span><span>${e(v.channel)}</span></div><h3>${e(v.title)}</h3><p>${e(v.description)}</p><h4>As you watch, look for</h4><ul>${(v.focus || []).map((f) => `<li>${e(f)}</li>`).join('')}</ul><a class="nv-source-link" href="https://www.youtube.com/watch?v=${v.id}" target="_blank" rel="noopener noreferrer">Watch on YouTube ↗</a></section>`;
    }
    function mechanism(c) {
        const v = c.visual;
        return `<section class="nv-mechanism"><div class="nv-section-label"><span>HOW IT SCALES</span><span>Concept · not to scale</span></div><div class="nv-diagram-stage" style="--nv-accent:${tone(c)}">${diagram(c)}</div><h3>${e(v?.title || c.mechanism)}</h3><p>${e(v?.description || c.thought)}</p><div class="nv-mechanism-steps">${(v?.steps || []).map((s, i) => `<span><b>${String(i + 1).padStart(2, '0')}</b>${e(s)}</span>`).join('')}</div></section>`;
    }
    function gallery() {
        const companies = neuroIdeasData.companies;
        return `<section class="nv-gallery"><div class="section-heading"><div><span class="eyebrow">MEET THE INTERFACES</span><h2>Six approaches. Different tradeoffs.</h2></div><a href="${companyUrl('neuralink')}">Explore the visual guide ↗</a></div><div class="nv-gallery-grid">${companies.map((c) => `<a class="nv-gallery-card" href="${companyUrl(c.id)}">${image(c, true)}<span><strong>${e(c.visual?.name || c.name)}</strong><small>${e(c.visual?.route || c.mechanism)}</small></span><i aria-hidden="true">↗</i></a>`).join('')}</div></section>`;
    }
    function profile(c) {
    ...
    function bind(container) {
        container.querySelectorAll('[data-neuro-image]').forEach(img => {
            if (img.dataset.neuroBound)
                return;
            img.dataset.neuroBound = 'true';
            const failed = () => { img.hidden = true; img.parentElement?.classList.add('nv-image-unavailable'); };
            img.addEventListener('error', failed, { once: true });
            if (img.complete && img.naturalWidth === 0)
                failed();
        });
        container.querySelectorAll('[data-neuro-video]').forEach(button => {
            if (button.dataset.neuroBound)
                return;
            button.dataset.neuroBound = 'true';
            button.addEventListener('click', () => {
                ...
                window.setTimeout(() => {
                    ...
                    let blank = false;
                    try {
                        blank = frame.contentDocument?.URL === 'about:blank';
                    }
                    catch (_) {
                        return;
                    }
                    ...
                    status.className = 'nv-player-status';
    ...
    function resetPlayers(container) {
        container.querySelectorAll('.nv-video-frame iframe').forEach(frame => {
