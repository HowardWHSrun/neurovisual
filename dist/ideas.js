const NeuroIdeas = (() => {
    ...
    const head = (eyebrow, title, text) => `<header class="hub-page-head idea-page-head"><div><div class="eyebrow">${e(eyebrow)}</div><h1>${e(title)}</h1><p>${e(text)}</p></div></header>`;
    const card = (x, index) => `<a class="idea-note-row" href="${href(x.id)}"><span class="idea-note-number" aria-hidden="true">${String(index + 1).padStart(2, '0')}</span><div><span class="idea-note-theme">${e(x.theme)}</span><h3>${e(x.title)}</h3><p>${e(x.question)}</p></div><span class="idea-note-state">${e(x.status)}<span aria-hidden="true">↗</span></span></a>`;
    function records() {
    ...
    function notebook() {
        return `<div class="idea-notebook"><header class="idea-notebook-head"><div><span class="eyebrow">RESEARCH NOTEBOOK</span><h1>Big ideas, made testable.</h1></div><p>${ideas.length} open notes<br><span>Reviewed ${e(neuroIdeasData.reviewed)}</span></p></header>
      <section class="idea-feature" aria-labelledby="idea-feature-title"><div class="idea-feature-question"><span class="eyebrow">THE STARTING OBSERVATION</span><h2 id="idea-feature-title">What exactly is doubling?</h2><p>A lab moves from a few probes to thousands of channels. The counting unit can change along the way.</p><a class="button-primary" href="${href('moores-law-bci')}?section=counts">Examine the comparison →</a></div><div class="idea-feature-comparison"><span class="idea-comparison-label">NEUROPIXELS · 2017 → 2025 · PER PROBE</span><dl><div><dt>Electrode sites</dt><dd>960 <span aria-hidden="true">→</span> 6,144</dd><p>6.4× more selectable sites</p></div><div><dt>Simultaneous channels</dt><dd>384 <span aria-hidden="true">→</span> 384</dd><p>The same concurrent count</p></div></dl><p class="idea-feature-conclusion"><strong>2.99 years per doubling</strong> for sites between these endpoints. A different metric tells a different story.</p></div></section>
      ...
      <section class="idea-notebook-index" aria-labelledby="idea-index-title"><div class="idea-index-heading"><div><span class="eyebrow">QUESTIONS TO KEEP DEVELOPING</span><h2 id="idea-index-title">The notebook</h2></div><span>Observation → evidence → next test</span></div><div class="idea-note-list">${ideas.map(card).join('')}</div></section>
      ${NeuroMedia.gallery()}
      <section class="idea-lab-bridge"><div><span class="eyebrow">FOLLOW THE UNIVERSITY RESEARCH</span><h2>What else is scaling?</h2><p>Stable recording, selective stimulation, neural decoding, imaging, and functional recovery.</p></div><a href="#labs?view=progress">Explore lab progress →</a></section>
      <aside class="idea-editorial"><h2>From a thought to an experiment</h2><p>Capture the observation. Attach the original evidence. State an interpretation, identify a test, and preserve what changed when new evidence arrives.</p><p>${e(neuroIdeasData.editorial_note)}</p></aside></div>`;
    }
    function thought(x) {
        return `<div class="idea-with-visual">${NeuroVisuals.idea(x.id)}<div class="idea-reading"><section class="idea-observation"><span class="idea-step-label"><b aria-hidden="true">01</b> OBSERVATION</span><h2>What prompted the question</h2><p>${e(x.observation)}</p></section><section class="idea-inference"><span class="idea-step-label"><b aria-hidden="true">02</b> WORKING INTERPRETATION</span><h2>A research thought</h2><p>${e(x.interpretation)}</p></section><section class="idea-test"><span class="idea-step-label"><b aria-hidden="true">03</b> MAKE IT TESTABLE</span><h2>The next comparison</h2><p>${e(x.test)}</p><div class="idea-revision"><h3>What would change this view?</h3><p>${e(x.revision)}</p></div></section></div></div>`;
    }
    function countExplorer(params) {
        ...
        const bars = rows.map((r) => `<div class="idea-bar-row"><div class="idea-bar-name"><strong>${e(r.name)}</strong><span>${r.year}${r.id === 'np_quad_2026' ? ' preprint' : ''} · ${r.shanks} ${r.shanks === 1 ? 'shank' : 'shanks'}</span></div><div class="idea-bar-track" aria-hidden="true"><span style="width:${r[key] / maximum * 100}%"></span></div><strong class="idea-bar-value">${n(r[key])}</strong><a href="${hubUtils.sourceHref(r.source)}" target="_blank" rel="noopener noreferrer" aria-label="Source for ${e(r.name)}">Source ↗</a></div>`).join('');
        return `<div class="idea-count-workspace"><section class="idea-count-chart" aria-labelledby="idea-chart-title"><div class="idea-chart-header"><div><span class="eyebrow">ONE FAMILY · PUBLISHED CONFIGURATIONS</span><h2 id="idea-chart-title">${metric === 'sites' ? 'Sites available on a probe' : 'Channels recorded at once'}</h2></div><div class="idea-toggle" role="group" aria-label="Count metric"><button type="button" id="idea-sites" data-idea-param="metric" data-value="sites" aria-pressed="${metric === 'sites'}">Electrode sites</button><button type="button" id="idea-channels" data-idea-param="metric" data-value="channels" aria-pressed="${metric === 'channels'}">Simultaneous channels</button></div></div><figure class="idea-count-figure"><figcaption>Selected configurations, counted per probe.<span>0 — ${n(maximum)} ${metric === 'sites' ? 'sites' : 'channels'} · linear scale</span></figcaption><div class="idea-bars">${bars}</div></figure><div class="idea-count-takeaway"><span class="idea-takeaway-number">${metric === 'sites' ? '6.4×' : '384 → 384'}</span><p>${metric === 'sites' ? '<strong>More sites between 2017 and 2025.</strong> Eight rounded publication years imply 2.99 years per doubling. The intermediate designs do not establish a regular cadence.' : '<strong>The same channel count in the 2017 and 2025 endpoints.</strong> Quad Base expands this to 1,536; its 2026 preprint follows August 2025 commercial availability.'}</p></div><p class="idea-footnote">Quad Base is a four-shank design, not the successor to single-shank Ultra. This is a comparison of published configurations, not a fitted technology curve. ${source({ title: 'Quad Base availability', url: 'https://www.neuropixelscentral.org/technology' })}</p></section>
      <aside class="idea-count-definitions" aria-labelledby="idea-count-key"><h2 id="idea-count-key">Define the count</h2>${[['Probes', 'Physical threads, shafts, or assembled arrays: specify which.'], ['Sites', 'Fabricated or implanted contacts, including any reference/stimulation roles.'], ['Channels', 'Signals acquired concurrently at a stated rate and bandwidth.'], ['Neurons', 'Quality-assessed units; neither one site nor one channel equals one neuron.']].map(([a, b], i) => `<div class="${(metric === 'sites' && i === 1) || (metric === 'channels' && i === 2) ? 'is-active' : ''}"><h3><span aria-hidden="true">${String(i + 1).padStart(2, '0')}</span>${a}</h3><p>${b}</p></div>`).join('')}</aside></div>
      ${NeuroVisuals.explainer()}<section class="idea-scenario" aria-labelledby="idea-scenario-title"><div class="idea-scenario-control"><span class="eyebrow">ARITHMETIC, NOT A FORECAST</span><h2 id="idea-scenario-title">Try a doubling time</h2><label for="idea-doubling">Assumed doubling time <strong><output id="idea-doubling-value" for="idea-doubling">3</output> years</strong></label><input id="idea-doubling" type="range" min="2" max="10" step="0.1" value="3"><div class="idea-range-labels" aria-hidden="true"><span>2 years</span><span>10 years</span></div></div><div class="idea-scenario-result"><span>Relative growth after 12 years</span><strong id="idea-growth" aria-live="polite">16.0×</strong><p>2<sup>12 ÷ doubling time</sup></p></div><div class="idea-scenario-reference"><span class="eyebrow">HISTORICAL REFERENCE</span><p>The 2011 estimate was <strong>7.4 years</strong> for recorded neurons: <strong>3.08×</strong> over the same 12-year interval.</p>${source({ title: 'Stevenson & Kording · 2011', url: 'https://doi.org/10.1038/nn.2731' })}</div></section>`;
    }
    function companyExplorer(params) {
      ...
      <div class="nv-story-media">${NeuroMedia.mechanism(selected)}${selected.media?.image?.kind !== 'video-preview' ? NeuroMedia.video(selected) : NeuroMedia.watchNotes(selected)}</div>
    ...
    function bandwidth() {
        return `<section class="idea-budget" aria-labelledby="idea-budget-title"><div class="idea-budget-controls"><span class="eyebrow">ACQUISITION THOUGHT EXPERIMENT</span><h2 id="idea-budget-title">The raw data budget</h2><p>Set the acquisition parameters.</p><div class="idea-budget-inputs"><label for="idea-budget-channels"><span>Channels <small>1–1,000,000</small></span><input id="idea-budget-channels" type="number" inputmode="numeric" min="1" max="1000000" step="1" value="1024"></label><label for="idea-budget-rate"><span>Samples / second / channel <small>1–1,000,000</small></span><input id="idea-budget-rate" type="number" inputmode="numeric" min="1" max="1000000" step="1" value="30000"></label><label for="idea-budget-bits"><span>Bits / sample</span><select id="idea-budget-bits"><option>8</option><option>12</option><option selected>16</option><option>24</option><option>32</option></select></label></div></div><div class="idea-budget-display"><div class="idea-budget-formula" aria-hidden="true"><span>N<small>channels</small></span><b>×</b><span>f<small>samples / s</small></span><b>×</b><span>b<small>bits / sample</small></span></div><p class="idea-budget-output" role="status" aria-live="polite" aria-atomic="true"><strong id="idea-budget-result">491.52 Mb/s · 221.18 GB/hour</strong><span>Uncompressed throughput · hourly storage<br>Decimal units</span></p><p class="idea-budget-boundary">Excludes headers, compression, redundancy, and processing. Raw bit rate is not neural information rate.</p></div></section>`;
    }
    function render(id, params = new URLSearchParams()) {
        ...
        return `<div class="idea-article-head ${main && section === 'companies' ? 'nv-compact-heading' : ''}"><a class="back-link" href="#ideas">← All ideas</a>` + head(x.theme, x.title, x.question) + '</div>'
            + `<div class="idea-meta idea-article-meta"><span class="idea-status">${e(x.status)}</span><span>Updated ${e(x.updated)}</span></div>`
            + (main ? `<nav class="idea-tabs" aria-label="Moore’s law sections">${[['thought', 'The idea'], ['counts', 'Compare counts'], ['companies', 'Company strategies'], ['evidence', 'Evidence & sources']].map(([v, label], i) => `<button type="button" id="idea-section-${v}" data-idea-param="section" data-value="${v}" aria-pressed="${v === section}"><span aria-hidden="true">${String(i + 1).padStart(2, '0')}</span>${label}</button>`).join('')}</nav>` : '')
            + (main && section === 'counts' ? countExplorer(params) : main && section === 'companies' ? companyExplorer(params) : main && section === 'evidence' ? evidence() : (x.id === 'readout-budget' ? bandwidth() : '') + thought(x) + sources(x.sources))
