const AtlasVisuals = (() => {
    ...
    const tones = { electrode: '#26718c', field: '#387899', imaging: '#397f70', stimulate: '#a35d79', decode: '#7362a4', feedback: '#b07732', tissue: '#527d57', workflow: '#527188' };
    const labels = {
    ...
    function mode(d) {
        const id = d?.id || '';
        if (['calcium', 'voltage', 'neurotransmitter', 'twophoton', 'fmri', 'fnirs', 'pet', 'fus', 'clearing', 'connectome', 'cellatlas'].includes(id))
            return 'imaging';
        if (['eeg', 'hdeeg', 'meg', 'ecog', 'layer7', 'stentrode', 'consumer', 'openbci', 'wearable'].includes(id))
            return 'field';
        if (['utah', 'silicon', 'neuropixels', 'net', 'mesh', 'n1', 'connexus'].includes(id))
            return 'electrode';
        if (['motorbci', 'bionichand'].includes(id))
            return 'feedback';
        if (['organoid', 'brainoware', 'scaffold', 'living'].includes(id))
            return 'tissue';
        if (['dbs', 'icms', 'stimnet', 'corticalstim', 'ees', 'fes', 'vns', 'tes', 'consumertes'].includes(id))
            return 'stimulate';
        if (['spikesort', 'decoder', 'neuralcode', 'digitaltwin', 'neuromorphic'].includes(id))
            return 'decode';
        return 'workflow';
    }
    function art(kind) {
        const node = (x, y, r = 6) => `<circle cx="${x}" cy="${y}" r="${r}" class="av-dot"/>`;
        const neurons = (x, y) => `<g transform="translate(${x} ${y})"><path d="M0 0 26 20 12 55M26 20 57 10 81 35M26 20 47 57 81 66M57 10 68-7M12 55 0 79"/>${[[0, 0], [26, 20], [12, 55], [57, 10], [81, 35], [47, 57], [81, 66]].map(p => node(p[0], p[1], 4)).join('')}</g>`;
        const trace = (x, y) => `<path d="M${x} ${y}h15l7-8 7 17 7-34 7 40 7-15h20"/>`;
        const arrow = (x, y) => `<path d="M${x} ${y}h27m-7-6 7 6-7 6" class="av-arrow"/>`;
        let drawing = '';
        if (kind === 'electrode')
            drawing = `<path class="av-wash" d="M17 87Q55 62 98 86t63 0v118H17Z"/>${neurons(28, 120)}<rect x="92" y="31" width="22" height="145" rx="8" class="av-device"/>${[65, 91, 117, 143].map(y => node(103, y, 4)).join('')}<path d="M103 31V17h136v37"/><rect x="191" y="55" width="82" height="112" rx="9" class="av-device"/>${[75, 93, 111, 129, 147].map(y => `<path d="M182 ${y}h9m82 0h9"/>`).join('')}${arrow(288, 112)}${[80, 116, 152].map(y => trace(326, y)).join('')}`;
        if (kind === 'field')
        ...
        if (kind === 'imaging')
            drawing = `<rect x="20" y="60" width="115" height="119" rx="18" class="av-wash"/>${neurons(36, 89)}${arrow(147, 118)}<rect x="194" y="68" width="81" height="105" rx="8" class="av-device"/><circle cx="234" cy="119" r="26"/><path d="M213 119h42m-21-21v42"/>${arrow(289, 117)}${Array.from({ length: 25 }, (_, i) => `<rect x="${330 + i % 5 * 14}" y="${80 + Math.floor(i / 5) * 14}" width="11" height="11" rx="2" fill="currentColor" opacity="${[.17, .35, .65, .45, .85][(i * 3 + Math.floor(i / 5)) % 5]}" stroke="none"/>`).join('')}`;
        if (kind === 'stimulate')
            drawing = `<rect x="24" y="60" width="101" height="117" rx="12" class="av-device"/><path d="M42 91h64m-64 27h64m-64 27h64"/>${node(60, 91)}${node(89, 118)}${node(72, 145)}${arrow(138, 117)}<rect x="188" y="68" width="80" height="103" rx="10" class="av-device"/><circle cx="228" cy="119" r="24"/><circle cx="228" cy="119" r="9"/>${arrow(282, 117)}<rect x="326" y="60" width="91" height="117" rx="12" class="av-device"/><path d="M342 80v77h59m-53-25 14-16 12 8 19-24"/>`;
        if (kind === 'decode')
            drawing = `${[77, 117, 157].map(y => trace(22, y)).join('')}${arrow(109, 118)}<rect x="155" y="49" width="124" height="137" rx="12" class="av-device"/>${[[177, 77], [185, 90], [178, 101], [205, 128], [216, 119], [218, 139], [249, 81], [256, 99], [241, 94]].map(p => node(p[0], p[1], 4)).join('')}<path d="m173 171 81-104" class="av-dashed"/>${arrow(291, 118)}<rect x="332" y="58" width="78" height="116" rx="9" class="av-device"/><path d="M346 84h47M346 96h34M346 143h47"/><path d="m360 121 12 12 21-30"/>`;
        if (kind === 'feedback')
            drawing = `${neurons(25, 88)}${arrow(116, 116)}<rect x="164" y="67" width="85" height="100" rx="10" class="av-device"/><path d="M180 91h52m-52 15h35m-35 34h52"/>${arrow(264, 116)}<path d="m315 149 12-18-4-44q4-12 11 0l7 27 5-52q5-10 10 0l1 49 7-42q7-9 11 3l-3 44 10-30q9-7 11 4l-11 66-16 20h-29Z" class="av-device"/><path d="M360 188v26H64v-33m-6 7 6-7 6 7" class="av-dashed"/>`;
        if (kind === 'tissue')
            drawing = `<path d="m23 131 66-43 60 34-67 45Z" class="av-device"/><path d="m23 145 59 34 67-44m-126 25 59 34 67-44M44 120l59 35M65 106l59 35M50 147l65-43M72 158l65-43"/>${arrow(158, 118)}<path d="M203 67h75v95q-38 20-75 0Z" class="av-wash"/><ellipse cx="240" cy="68" rx="38" ry="12"/>${[[218, 111], [247, 91], [264, 128], [236, 145], [212, 149]].map(p => `<ellipse cx="${p[0]}" cy="${p[1]}" rx="8" ry="6"/>`).join('')}${arrow(290, 118)}${trace(331, 106)}<path d="M332 153h74m-68 0v-15m16 15v-28m16 28v-44m16 44v-25"/>`;
        if (kind === 'workflow')
            drawing = `${[27, 174, 321].map((x, i) => `<rect x="${x}" y="57" width="94" height="125" rx="11" class="av-device"/><circle cx="${x + 47}" cy="95" r="19" class="av-wash"/>${i === 0 ? `<path d="M${x + 40} 88q0-11 12-7t-3 18v5m0 8v2"/>` : i === 1 ? `<path d="M${x + 34} 98l9-12 9 15 9-8"/>` : `<path d="m${x + 35} 95 8 8 17-20"/>`}<path d="M${x + 20} 139h54m-54 14h36"/>`).join('')}${arrow(132, 119)}${arrow(279, 119)}`;
    ...
    function schematic(kind, title, detail, source, steps) {
    ...
    function labMedia(orgId) { const ids = neuroLabsData.labs.filter(l => l.atlasIds.includes(orgId)).map(l => 'lab-' + l.id); return NeuroVisuals.items().find(m => ids.includes(m.id)); }
    function methodVisual(d) {
        const exact = { eeg: 'eeg', hdeeg: 'eeg', meg: 'meg', ecog: 'ecog', fmri: 'fmri', fnirs: 'fnirs', calcium: 'calcium', utah: 'intracortical', silicon: 'intracortical', neuropixels: 'intracortical', net: 'intracortical', mesh: 'intracortical', n1: 'intracortical', connexus: 'intracortical' };
        if (exact[d?.id])
            return FieldVisuals.method(exact[d.id]);
        if (d?.g === 'stimulate' || d?.o === 'write' || mode(d) === 'stimulate')
            return FieldVisuals.topic('stimulation');
        if (d?.id === 'seeg')
            return schematic('workflow', d.n, tx('Depth contacts sample intracranial potentials along selected trajectories. This acquisition flow does not depict contact geometry, cortical-surface placement, or an isolated neuron.', '深部触点沿选定轨迹采集颅内电位。此采集流程不表示触点几何结构、皮层表面放置方式或单个分离神经元。'), undefined, [tx('Depth contacts', '深部触点'), tx('Acquisition', '信号采集'), tx('Potentials', '电位')]);
        return '';
    }
    function technology(d, sourceLinks = []) {
        const mediaIds = { utah: 'company-blackrock', n1: 'company-neuralink', layer7: 'company-precision', connexus: 'company-paradromics', stentrode: 'company-synchron', net: 'lab-rice-xie', stimnet: 'lab-rice-luan', speechspike: 'lab-stanford-nptl', speechecog: 'lab-ucsf-chang', brainspine: 'lab-epfl-courtine' };
        const m = mediaIds[d.id] && NeuroVisuals.items().find(m => m.id === mediaIds[d.id]);
        if (m)
            return media(m, tx('RELATED PLATFORM OR STUDY · SEE CAPTION', '相关平台或研究 · 详见图注'));
        const method = methodVisual(d);
        if (method)
            return method;
    ...
    function organization(d, related = []) {
        const company = NeuroIdeas.companyForOrg(d.id);
        const companyImage = company && NeuroVisuals.items().find(m => m.id === 'company-' + company.id);
        if (companyImage)
            return media(companyImage, tx('VERIFIED PLATFORM IMAGE · SEE CAPTION', '已核实的平台图像 · 详见图注'));
        const m = labMedia(d.id);
        if (m)
            return media(m, tx('RESEARCH FROM THIS LAB', '本实验室研究'));
        const t = related[0];
        const method = methodVisual(t);
        if (method)
            return method;
    ...
    function researcher(d, related = []) {
        ...
        if (m)
            return media(m, tx('A RESEARCH EXAMPLE FROM THIS INVESTIGATOR’S LAB', '这位研究者所在实验室的研究实例'));
        const t = related[0];
        const method = methodVisual(t);
        if (method)
            return method;
    ...
    function mount(container, markup, selector) {
        const anchor = container.querySelector(selector);
        if (!anchor || !markup)
            return;
        const pair = document.createElement('div');
        pair.className = 'av-context-pair';
        const visual = document.createElement('div');
        visual.className = 'av-context-visual';
        visual.innerHTML = markup;
        anchor.before(pair);
        pair.append(visual, anchor);
        anchor.classList.add('av-context-copy');
