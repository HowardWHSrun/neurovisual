const PeopleWorkplaces = (() => {
    ...
    const areas = { record: 'Recording & imaging', stimulate: 'Stimulation & modulation', restore: 'Restoration & clinical interfaces', compute: 'Computation & decoding', personal: 'Ethics & human experience', translation: 'Technology translation' };
    ...
    const directory = (params) => params.get('view') === 'people' || (params.get('view') !== 'map' && !['focus', 'node', 'edge', 'relation', 'overview'].some(key => params.has(key)));
    const href = (params, updates = {}) => {
        const next = new URLSearchParams({ by: 'people', view: 'people' });
        for (const key of ['q', 'area', 'workplace', 'sort', 'page'])
            if (params.get(key))
    ...
    function bind(container, params, navigate) {
        const form = container.querySelector('#pw-filter');
        form?.addEventListener('submit', event => {
            event.preventDefault();
            navigate(href(params, { q: container.querySelector('#pw-query').value.trim(), area: container.querySelector('#pw-area').value, workplace: container.querySelector('#pw-workplace').value, sort: container.querySelector('#pw-sort').value, page: '' }), 'pw-query');
        });
        container.querySelectorAll('[data-pw-nav],[data-pw-map]').forEach(link => link.addEventListener('click', event => {
