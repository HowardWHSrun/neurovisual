var NeuroLabs = (() => {
    ...
    const readLabel = (status) => ({ 'read': 'Page content read', 'abstract-only': 'Abstract / publication record read', 'unavailable': 'Page unavailable' }[status] || 'Review scope not recorded');
    function sourceReview(s) { return s.review ? `<small class="lab-source-review" data-review="${e(s.review.status)}">${e(readLabel(s.review.status))} · checked ${e(s.review.checkedAt)}</small><p class="lab-source-note">${e(s.review.note)}</p>` : '<small>Source not rechecked in this audit</small>'; }
    function auditPanel(l) { const a = l.audit; if (!a)
        return ''; return `<section class="lab-audit" aria-label="Website evidence review"><div class="lab-audit-top"><span class="eyebrow">WEBSITE REVIEW · ${e(l.verified)}</span><span class="lab-audit-status">${a.status === 'partial' ? 'Review has unresolved gaps' : a.pages.some(p => p.status === 'unavailable') ? 'Some pages unavailable' : 'Source review documented'}</span></div><div class="lab-audit-grid"><div><h2>What the source review found</h2><p>${e(a.summary)}</p></div><div class="lab-audit-latest"><span>Newest dated source found</span>${a.latest ? `<time>${e(a.latest.date)}</time>${ext(a.latest.url, a.latest.title)}` : '<p>No publication or update date established from the pages read.</p>'}</div></div><details><summary>Review notes &amp; pages checked (${a.pages.length})</summary><ul class="lab-audit-changes">${a.changes.map(c => `<li>${e(c)}</li>`).join('')}</ul><ol class="lab-audit-pages">${a.pages.map(p => `<li>${ext(p.url, p.title)}<small data-review="${e(p.status)}">${e(readLabel(p.status))} · ${e(p.checkedAt)}</small><p>${e(p.supports)}</p></li>`).join('')}</ol><p class="lab-audit-scope">This is a dated review of these pages. “Newest found” does not mean every recent paper was located. An abstract review does not establish that the full paper was read.</p></details></section>`; }
   ...
   <div class="lab-school-index">${institutions.map(institution => {
    ...
    function directory(params) {
        ...
        if (view === 'coverage')
            return `${head}${coverage()}</div>`;
        if (view === 'schools')
            return `${head}${schoolsPage(params)}</div>`;
   ...
   ${NeuroConnections.teaser('#labs/' + l.id)}<section class="lab-related"><div class="section-heading"><h2>Related research to explore</h2><a href="#labs">All labs →</a></div><p>Connected by shared research areas or institution; this does not imply collaboration.</p><div class="lab-grid">${related.map(x => card(x.lab)).join('')}</div></section></article>`;
    ...
    function bind(container, id, params, update) {
        if (id) {
            container.querySelectorAll('[data-lab-section]').forEach(button => button.addEventListener('click', () => {
                const target = container.querySelector('#lab-profile-' + button.dataset.labSection);
                if (!target)
                    return;
