var NeuroLabs = (() => {
    ...
    function card(l) {
        return `<article class="lab-card" style="--lab-color:${color(l)}">
   ...
   <div class="lab-school-index">${institutions.map(institution => {
    ...
    function directory(params) {
        ...
        return `${head}${note}${filters(params, view)}${areas}${results}${all.length ? rows : empty(view)}${pages}<p class="lab-footnote">Area totals overlap because a lab can work across several fields. Profiles summarize selected programs; university directories help you explore the wider roster.</p></div>`;
    }
    function coverage() {
