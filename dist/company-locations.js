const CompanyLocations = (() => {
    ...
    const pageSize = 12;
    const colors = { 'North America': '#347dc2', Europe: '#2b957c', Asia: '#c47739', 'Middle East': '#aa5d92', Oceania: '#7b64bd', 'Latin America': '#348f9c', 'Location not listed': '#66788d' };
    let observer;
    const countryOf = (c) => c.country?.trim() || 'Country not listed';
    const cityOf = (c) => c.city?.trim() || 'City not listed';
    const regionOf = (c) => c.region?.trim() || 'Location not listed';
    const count = (n) => `${n} ${n === 1 ? 'company' : 'companies'}`;
    const companies = () => (window.neuroAtlas?.organizations || []).filter(c => ['Company', 'Startup', 'Public company'].includes(c.kind));
    function link(params, patch = {}) {
        const next = new URLSearchParams({ by: 'countries', view: 'companies' });
        for (const key of ['country', 'city', 'q', 'sort', 'page']) {
    ...
    function model(params) {
        ...
        const groups = [];
        const add = (groupName, region, name, href) => {
            let group = groups.find(g => g.name === groupName);
            if (!group) {
                group = { name: groupName, region, places: [] };
                groups.push(group);
            }
            let place = group.places.find(p => p.href === href);
            if (!place) {
                place = { name, href, region, count: 0 };
        ...
        for (const c of rows) {
            if (city)
                add(city, regionOf(c), c.name, '#org/' + encodeURIComponent(c.id));
            else if (country)
                add(country, regionOf(c), cityOf(c), link(params, { city: cityOf(c), page: '' }));
            else
                add(regionOf(c), regionOf(c), countryOf(c), link(params, { country: countryOf(c), city: '', page: '' }));
        }
        for (const g of groups)
            g.places.sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
    ...
    function layout(groups, width, height) {
        if (!groups.length)
            return [];
    ...
    function chart(groups, width, height) {
        const box = (n) => `left:${n.x0 / width * 100}%;top:${n.y0 / height * 100}%;width:${(n.x1 - n.x0) / width * 100}%;height:${(n.y1 - n.y0) / height * 100}%;`;
        return layout(groups, width, height).map(g => {
            ...
            return `<div class="cl-region${g.y1 - g.y0 < 96 ? ' cl-region-compact' : ''}" style="${box(g)}--cl-color:${color}"><span>${e(g.data.name)}</span></div>` + g.children.map(n => {
                const p = n.data, small = n.x1 - n.x0 < 90 || n.y1 - n.y0 < 52;
    ...
    function render(params = new URLSearchParams()) {
        ...
        const options = (values, value) => [...(value && !values.includes(value) ? [value] : []), ...values].map(v => `<option value="${e(v)}"${v === value ? ' selected' : ''}>${e(v)}</option>`).join('');
    ...
    function bind(container, params, navigate) {
        close();
        const page = container.querySelector('.cl-page');
        if (!page)
            return;
        page.addEventListener('click', event => {
            const a = event.target.closest('[data-cl-nav]');
            if (!a || event.button !== 0 || event.metaKey || event.ctrlKey || event.altKey || event.shiftKey)
                return;
            const href = a.getAttribute('href');
            if (!href.startsWith('#explore?'))
                return;
            event.preventDefault();
            navigate(href, a.dataset.clFocus || 'cl-title');
            document.getElementById(a.dataset.clFocus || 'cl-title')?.scrollIntoView({ block: 'start' });
        });
        const title = page.querySelector('#cl-title');
        title?.setAttribute('tabindex', '-1');
        const form = page.querySelector('#cl-filters'), country = page.querySelector('#cl-country'), city = page.querySelector('#cl-city'), query = page.querySelector('#cl-query');
        const update = (focus, resetCity = false) => navigate(link(params, { q: query.value.trim(), country: country.value, city: resetCity ? '' : city.value, page: '' }), focus);
        ...
        const sort = page.querySelector('#cl-sort');
        sort.addEventListener('change', () => navigate(link(params, { sort: sort.value, page: '' }), 'cl-sort'));
        const map = page.querySelector('#cl-treemap'), hint = page.querySelector('#cl-map-hint');
        if (map) {
            const selected = model(params), groups = selected.groups;
            const showHint = (event) => { const cell = event.target.closest('[data-cl-hint]'); if (cell && hint)
                hint.textContent = cell.dataset.clHint; };
            map.addEventListener('pointerover', showHint);
            map.addEventListener('focusin', showHint);
            let lastWidth = 0;
            const resize = () => {
                ...
                if (!width || width === lastWidth)
                    return;
                lastWidth = width;
                const active = document.activeElement, href = map.contains(active) ? active.getAttribute('href') : null;
                const height = selected.city ? Math.min(470, Math.max(190, Math.ceil(selected.rows.length / (width < 600 ? 2 : 4)) * 100)) : width < 600 ? 520 : 470;
                map.style.height = height + 'px';
                map.innerHTML = chart(groups, width, height);
                if (href)
                    Array.from(map.querySelectorAll('a')).find(a => a.getAttribute('href') === href)?.focus({ preventScroll: true });
            };
            resize();
    ...
    function records() { return [{ id: 'company-locations', title: 'Companies by location', description: 'Explore neurotechnology companies by country and city, with company profiles and career links.', kind: 'Company locations', href: link(new URLSearchParams()), keywords: 'jobs careers employers country city region location treemap' }]; }
