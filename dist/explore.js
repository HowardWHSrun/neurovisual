const NeuroExplore = (() => {
    ...
    const link = (by, params = {}) => '#explore?' + new URLSearchParams({ by, ...params }).toString();
    const lab = (id) => neuroLabsData.labs.find(l => l.id === id);
    const node = (id) => neuroConnectionsData.nodes.find(n => n.id === id);
    const problems = () => typeof neuroExplorationData !== 'undefined' ? neuroExplorationData.problems || [] : [];
    const paths = () => typeof neuroExplorationPeopleData !== 'undefined' ? neuroExplorationPeopleData.paths || [] : [];
    const visual = (id) => NeuroVisuals.items().find(m => m.id === id);
    const labVisual = (id) => visual('lab-' + id);
    const profile = (id) => '#labs/' + encodeURIComponent(id);
    const source = (url, title) => `<a href="${u(url)}" target="_blank" rel="noopener noreferrer">${e(title)} <span aria-hidden="true">↗</span></a>`;
    function photo(m, href, priority = false) {
        if (!m)
            return '';
    ...
    function intro(eyebrow, title, text) { return `<div class="ex-intro"><span class="eyebrow">${e(eyebrow)}</span><h2>${e(title)}</h2><p>${e(text)}</p></div>`; }
    ...
    function labRow(l, why = '', relationship = '', ref) {
    ...
    function problemPage(p, params) {
        const all = (p.labLinks || []).filter((x) => lab(x.labId));
        const selectedCountry = params.get('country') || '';
        const countryOptions = [...new Set(all.map((x) => lab(x.labId).country))].sort();
        const items = all.filter((x) => !selectedCountry || lab(x.labId).country === selectedCountry);
    ...
    function companyRow(c) { return `<article class="ex-lab-row"><div><span class="ex-label">COMPANY · ${e(c.city || c.country)}</span><h3><a href="#org/${encodeURIComponent(c.id)}">${e(c.name)} <span aria-hidden="true">→</span></a></h3><p>${e(c.summary)}</p></div>${c.source ? `<div class="ex-sources">${source(c.source, 'Original source')}</div>` : ''}</article>`; }
    function countryCount(x) { return `${x.labs.length} lab ${x.labs.length === 1 ? 'profile' : 'profiles'} · ${x.companies.length} ${x.companies.length === 1 ? 'company' : 'companies'}`; }
    function countries(params) {
        if (params.get('view') === 'companies')
            return CompanyLocations.render(params);
        const entries = countryEntries(), selected = entries.find(x => x.country === params.get('country'));
        if (selected) {
            const related = problems().map(p => ({ problem: p, count: (p.labLinks || []).filter((x) => lab(x.labId)?.country === selected.country).length })).filter(x => x.count);
    ...
    function render(params = new URLSearchParams()) {
        ...
        const countryViews = by === 'countries' ? `<nav class="ex-actions ex-country-views" aria-label="Location view"><a href="${e(link('countries', { view: 'companies' }))}" data-ex-nav${params.get('view') === 'companies' ? ' aria-current="true"' : ''}>Companies by location</a><a href="${e(link('countries'))}" data-ex-nav${params.get('view') !== 'companies' ? ' aria-current="true"' : ''}>Labs &amp; country guides</a></nav>` : '';
        return `<div class="ex-page" data-ex-view="${by}">${head(by)}${countryViews}${by === 'organizations' ? organizations() : by === 'people' ? (PeopleWorkplaces.directory(params) ? PeopleWorkplaces.render(params) : PeopleMap.render(params)) : by === 'countries' ? countries(params) : problemView(params)}</div>`;
    }
    function bind(container, _params, navigate) {
        ...
        const countrySelect = container.querySelector('#ex-problem-country');
        const updateCountry = () => { if (countrySelect)
        ...
        if (_params.get('by') === 'countries' && _params.get('view') === 'companies')
            CompanyLocations.bind(container, _params, navigate);
        NeuroVisuals.bind(container);
    }
    function records() {
        return [...CompanyLocations.records(), ...problems().map(p => ({ id: 'explore-problem-' + p.id, title: p.title, description: p.question + ' ' + p.summary, kind: 'Problem to solve', href: link('problems', { problem: p.id }), keywords: (p.labLinks || []).map((x) => lab(x.labId)?.name + ' ' + x.reason).join(' ') })), ...countryEntries().map(x => ({ id: 'explore-country-' + x.country, title: x.country + ' — labs & companies', description: countryCount(x) + ' mapped in this collection.', kind: 'Country guide', href: link('countries', { country: x.country }), keywords: x.labs.map(l => l.institution + ' ' + l.name + ' ' + l.leaders.join(' ')).concat(x.companies.map(c => c.name + ' ' + c.summary)).join(' ') })), ...paths().map(p => ({ id: 'explore-path-' + p.id, title: p.title, description: p.summary, kind: 'People connection', href: p.href, keywords: (p.people || []).map((id) => node(id)?.name || '').join(' ') }))];
    }
