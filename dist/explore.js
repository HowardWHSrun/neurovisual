const NeuroExplore = (() => {
    ...
    function records() {
        return [...problems().map(p => ({ id: 'explore-problem-' + p.id, title: p.title, description: p.question + ' ' + p.summary, kind: 'Problem to solve', href: link('problems', { problem: p.id }), keywords: (p.labLinks || []).map((x) => lab(x.labId)?.name + ' ' + x.reason).join(' ') })), ...countryEntries().map(x => ({ id: 'explore-country-' + x.country, title: x.country + ' — labs & companies', description: countryCount(x) + ' mapped in this collection.', kind: 'Country guide', href: link('countries', { country: x.country }), keywords: x.labs.map(l => l.institution + ' ' + l.name + ' ' + l.leaders.join(' ')).concat(x.companies.map(c => c.name + ' ' + c.summary)).join(' ') })), ...paths().map(p => ({ id: 'explore-path-' + p.id, title: p.title, description: p.summary, kind: 'People connection', href: p.href, keywords: (p.people || []).map((id) => node(id)?.name || '').join(' ') }))];
    }
